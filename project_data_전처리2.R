...
start1<- data %>% dplyr::select(-c("동","읍면동명_two","도로명","법규위반","도로형태","구","읍면동명","동면적"))
...
# start1$피해운전자연령<- as.integer(start1$피해운전자연령)
start2<- start1 %>% filter(피해운전자_연령>=65) %>% dplyr::select(-c("피해운전자_연령","통화량","횡단보도수_구_"))
...
############65세 이상 노인을 위한 회귀모델
start4<- start3 %>% filter(피해운전자연령>=65)

model_old<-glm(사고유형~. , data=start4)

summary(model)

###########어린이 피해사고 위한 회귀모델
start5<- start3 %>% filter(피해운전자연령<=15)

model_children<- glm(사고유형~. , data=start5)
